...
ures_swap(const UDataSwapper *ds,
    ...
    Row rows[ures_STACK_ROW_CAPACITY];
    int32_t resort[ures_STACK_ROW_CAPACITY];
    TempTable tempTable;
    ...
    if(length>=0) {
        ...
        uint32_t stackResFlags[ures_STACK_ROW_CAPACITY];
        ...
        if(tempTable.majorFormatVersion>1 || maxTableLength<=ures_STACK_ROW_CAPACITY) {
            tempTable.rows=rows;
