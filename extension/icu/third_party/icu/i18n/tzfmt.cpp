...
static const UChar UTZFMT_SINGLEQUOTE  = 0x0027;
static const UChar UTZFMT_PLUS         = 0x002B;
static const UChar UTZFMT_MINUS        = 0x002D;
...
TimeZoneFormat::parseOffsetISO8601(const UnicodeString& text, ParsePosition& pos, UBool extendedOnly, UBool* hasDigitOffset /* = NULL */) const {
    ...
    int32_t sign = 1;
    if (firstChar == UTZFMT_PLUS) {
        sign = 1;
    } else if (firstChar == UTZFMT_MINUS) {
        sign = -1;
    } else {
...
TimeZoneFormat::unquote(const UnicodeString& pattern, UnicodeString& result) {
    if (pattern.indexOf(UTZFMT_SINGLEQUOTE) < 0) {
...
TimeZoneFormat::parseOffsetPattern(const UnicodeString& pattern, OffsetFields required, UErrorCode& status) {
    ...
    for (int32_t i = 0; i < pattern.length(); i++) {
        ...
        if (ch == UTZFMT_SINGLEQUOTE) {
            if (isPrevQuote) {
                text.append(UTZFMT_SINGLEQUOTE);
                isPrevQuote = false;
            } else {
                isPrevQuote = true;
