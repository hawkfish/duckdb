...
DateIntervalInfo::initializeData(const Locale& locale, UErrorCode& status)
{
    ...
    calBundle = ures_getByKeyWithFallback(rb, dtitvinf_gCalendarTag, nullptr, &status);
    ...
    if (U_SUCCESS(status)) {
        ...
        itvDtPtnResource = ures_getByKeyWithFallback(calTypeBundle,
                                                     dtitvinf_gIntervalDateTimePatternTag, nullptr, &status);
...
DateIntervalInfo::getBestSkeleton(const UnicodeString& skeleton,
    ...
    if ( skeleton.indexOf(dtitv_impl_LOW_Z) != -1 || skeleton.indexOf(dtitv_impl_LOW_K) != -1 || skeleton.indexOf(dtitv_impl_CAP_K) != -1 || skeleton.indexOf(dtitv_impl_LOW_A) != -1 || skeleton.indexOf(dtitv_impl_LOW_B) != -1 ) {
        copySkeleton = skeleton;
        copySkeleton.findAndReplace(UnicodeString(dtitv_impl_LOW_Z), UnicodeString(dtitv_impl_LOW_V));
        copySkeleton.findAndReplace(UnicodeString(dtitv_impl_LOW_K), UnicodeString(dtitv_impl_CAP_H));
        copySkeleton.findAndReplace(UnicodeString(dtitv_impl_CAP_K), UnicodeString(dtitv_impl_LOW_H));
        copySkeleton.findAndReplace(UnicodeString(dtitv_impl_LOW_A), UnicodeString());
        copySkeleton.findAndReplace(UnicodeString(dtitv_impl_LOW_B), UnicodeString());
