...
static const UChar PK_VAR_E[]={PLURRULE_LOW_E,0};
static const UChar PK_VAR_C[]={PLURRULE_LOW_C,0};
...
PluralRuleParser::parse(const UnicodeString& ruleData, PluralRules *prules, UErrorCode &status)
{
    ...
    while (ruleIndex< ruleSrc->length()) {
        ...
        case plurrule_token_tVariableE:
        case plurrule_token_tVariableC:
...
static UnicodeString tokenString(tokenType tok) {
    ...
    switch (tok) {
      ...
      case plurrule_token_tVariableE:
        s.append(PLURRULE_LOW_E); break;
    case plurrule_token_tVariableC:
        s.append(PLURRULE_LOW_C); break;
      default:
...
PluralRuleParser::checkSyntax(UErrorCode &status)
{
    ...
    case plurrule_token_tColon:
        if (!(type == plurrule_token_tVariableN ||
              ...
              type == plurrule_token_tVariableE ||
              type == plurrule_token_tVariableC ||
...
PluralRuleParser::getKeyType(const UnicodeString &token, tokenType keyType)
{
    ...
    if (0 == token.compare(PK_VAR_N, 1)) {
        ...
        keyType = plurrule_token_tVariableE;
    } else if (0 == token.compare(PK_VAR_C, 1)) {
        keyType = plurrule_token_tVariableC;
