...
static TimeZoneRule* createRuleByRRULE(const UnicodeString& zonename, int rawOffset, int dstSavings, UDate start,
    ...
    int32_t doms[7];
    int32_t domsCount = UPRV_LENGTHOF(doms);
    UDate until;

    parseRRULE(rrule, month, dayOfWeek, nthDayOfWeek, doms, domsCount, until, status);
    if (U_FAILURE(status)) {
        return nullptr;
    }
    ...
    if (dates->size() == 1) {
        ...
        if (domsCount > 1) {
            ...
            if (domsCount != 7 || month == -1 || dayOfWeek == 0) {
                // Only support the rule using 7 continuous doms
            ...
            for (i = 0; i < 7; i++) {
                ...
                // as the base.
                if (doms[i] < 0) {
                    doms[i] = MONTHLENGTH[month] + doms[i] + 1;
                }
                if (doms[i] < firstDay) {
                    firstDay = doms[i];
                }
        ...
        if (month == -1 || dayOfWeek == 0 || domsCount == 0) {
